...
def apply_threshold(image):
    ...
    # yellow_lower = np.array([18, 45, 0], np.uint8)
    # yellow_upper = np.array([54, 255, 255], np.uint8)

    # Current
    yellow_lower = np.array([22, 69, 0], np.uint8)
    ...
    image_res = cv2.bitwise_and(image, image, mask=yellow)
...
def compare_lists(list_to_compare):
    ...
    # print("list_to_compare[0][1][0]", list_to_compare[0][2][0])
...
def grab_contour(threshold_image):
    ...
    # get two largest coutour areas
    for i, c in enumerate(cnts):
        area = cv2.contourArea(c)
        three_areas.append(area)
        sorteddata = sorted(zip(three_areas, cnts), key=lambda x: x[0], reverse=True)
    compare_list = [sorteddata[0][1][0][0], sorteddata[1][1][0][0], sorteddata[2][1][0][0]]

    first, second, third = compare_lists([compare_list])
    # print("three areas", three_areas)
    ...
    # cv2.circle(threshold_image, (423,159), 1, (100, 100, 100), thickness=10, lineType=8, shift=0)
    # cv2.imshow("leftmost--", threshold_image)
    # print("leftmost<>", leftmost_contour)
...
def compute_reference(reference_marker_image):
    ...
    print("ref_center_angle_1", center_angle_1)
    center_angle_2 = angle_change(center_point[0], center_point[2])
    print("ref_center_angle_2", center_angle_2)
    print("angle change test:", angle_change(center_point[0], center_point[1]))
    ...
    print("Distance between markers in ref markers", dist_markers)
    dist_markers = math.sqrt(
        (center_point[0][0] - center_point[2][0]) ** 2 + (center_point[0][1] - center_point[2][1]) ** 2)
    print("Distance between markers in ref markers", dist_markers)
    dist_markers = math.sqrt(
        (center_point[1][0] - center_point[2][0]) ** 2 + (center_point[1][1] - center_point[2][1]) ** 2)
    print("Distance between markers in ref markers", dist_markers)
    cv2.circle(reference_marker_image, (center_point[0]), 1, (100, 100, 100), thickness=3, lineType=8, shift=0)
    cv2.circle(reference_marker_image, (center_point[1]), 1, (100, 100, 100), thickness=3, lineType=8, shift=0)
    cv2.circle(reference_marker_image, (center_point[2]), 1, (100, 100, 100), thickness=3, lineType=8, shift=0)
    # cv2.imshow("reference1111111111", reference_marker_image)
...
def compute_moved(moved_marker_image):
    ...
    center_angle_1 = angle_change(center_point[0], center_point[1])
    print("moved_center_angle_1", center_angle_1)
    center_angle_2 = angle_change(center_point[0], center_point[2])
    print("moved_center_angle_2", center_angle_2)
    ...
    dist_markers = math.sqrt(
        (center_point[0][0] - center_point[1][0]) ** 2 + (center_point[0][1] - center_point[1][1]) ** 2)
    print("Distance between markers in moved markers", dist_markers)
    dist_markers = math.sqrt(
        (center_point[0][0] - center_point[2][0]) ** 2 + (center_point[0][1] - center_point[2][1]) ** 2)
    print("Distance between markers in moved markers", dist_markers)
    dist_markers = math.sqrt(
    ...
    cv2.circle(moved_marker_image, (center_point[0]), 1, (100, 100, 100), thickness=3, lineType=8, shift=0)
    cv2.circle(moved_marker_image, (center_point[1]), 1, (100, 100, 100), thickness=3, lineType=8, shift=0)
    cv2.circle(moved_marker_image, (center_point[2]), 1, (100, 100, 100), thickness=3, lineType=8, shift=0)
...
def marker_calculation():
    ...
    moved_marker_image = cv2.imread('images/1016_15m_transL_20.jpg')
    ...
    for i in range(0, 6):
        cv2.circle(moved_marker_image, tuple(moved_homography_points[i]), 1, (100, 100, 100), thickness=3, lineType=8, shift=0)
    ...
    for i in range(0, len(ref_center)):
        ...
        print("ref 0 1 2", getAngle(ref_center[0], ref_center[1], ref_center[2]))
        print("ref 1 2 0", getAngle(ref_center[1], ref_center[2], ref_center[0]))
        print("ref 2 0 1", getAngle(ref_center[2], ref_center[0], ref_center[1]))
        print("moved 0 1 2", getAngle(moved_center[0], moved_center[1], moved_center[2]))
        print("moved 1 2 0", getAngle(moved_center[1], moved_center[2], moved_center[0]))
