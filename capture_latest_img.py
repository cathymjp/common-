...
class App:
    def __init__(self, window, window_title, video_source=0):
        ...
        self.btn_snapshot = tkinter.Button(window, text="CAPTURE", width=20, height=2, command=self.snapshot,
                                           fg='white', bg='orange')
    ...
    def update(self):
        ...
        # captured image
        list_of_files = glob.glob('C:\\Users\Park\PycharmProjects\PyCharmCamera\\*.png')
        latest_file = max(list_of_files, key=os.path.getctime)
        print(latest_file)
        gif1 = tkinter.PhotoImage(file=latest_file)
        gif1 = gif1.subsample(2)
        ...
        if ret:
            ...
            # display last saved img
            self.canvas.create_image(730, 0, image=gif1, anchor=tkinter.NW)
            self.canvas.create_image(730, 250, image=gif1, anchor=tkinter.NW)
            self.canvas.create_image(1065, 0, image=gif1, anchor=tkinter.NW)
            self.canvas.create_image(1065, 250, image=gif1, anchor=tkinter.NW)
            self.window.after(self.delay, self.update)

            # self.canvas.create_image(700, 0, image=gif1, anchor=tkinter.SW)
            # self.window.after(self.delay, self.update)
